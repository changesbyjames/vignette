...
The Vite package also includes `src/virtual.d.ts` for its types-only `./virtual` entrypoint. The CLI
package includes `bin/vignette.js`, which loads the compiled CLI from `dist`.
...
React renderer, frames, Vite integration, MoQ extension, testkit, then CLI. `pnpm pack` produces the
final npm manifest with rewritten workspace dependencies; npm CLI publishes each tarball through
